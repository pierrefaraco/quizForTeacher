...
@ContextConfiguration(classes = { PersistenceJPAConfig.class, UserDaoImpl.class, TopicDaoImpl.class,
		SequenceDaoImpl.class, QuestionDaoImpl.class, QuizServiceImpl.class })
...
public class TestQuizService {
	...
	public void testCRUDSequence() {
		...
		SequenceDto sequenceDto = createSequenceDto(user, topics);
	...
	public void testListSequences() {
		...
		assertEquals(60 , mapQuestions.size());
		
		for (int i =0 ;i<50;i++){
	
			SequenceDto sequenceDto = createSequenceDto2(user,mapQuestions);
			quizService.createSequence(sequenceDto);	
	...
	public void testAddAndRemoveQuestionToSequence() {
		...
		topicDao.save(topic1);		
		topicDao.save(topic2);	
		SequenceDto sequenceDto = createSequenceDto2(user, null);
		...
		topics.put(topic1,10);
		topics.put(topic2,20);	
		sequenceDto = createSequenceDto(user,topics);
		quizService.createSequence(sequenceDto);
	...
	private SequenceDto createSequenceDto(User user, Map<Topic, Integer> topics) {
		Map<Integer, Question> mapQuestions = EntitiesCreator.createListOfQuestions(questionDao, topics);
		return   createSequenceDto2(user,  mapQuestions );
	}
	
	private SequenceDto createSequenceDto2( User user, Map<Integer, Question> mapQuestions ) {	
